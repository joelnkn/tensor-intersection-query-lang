"""
indirect einsum from github.com/nullplay
"""
...
class EinsumNode(ASTNode):
    ...
    def run(self, localenv, shapeenv):
        ...
        einsum_str = (
            f"{','.join(einsum_input_indices)}->{''.join(einsum_output_indices)}"
        )
...
class AssignmentNode(ASTNode):
    ...
    def run(self, localenv, shapeenv):
        ...
        final_output = eval_scatter_dst.index_put_(
            tuple(eval_scatter_indices), eval_scatter_src, accumulate=self.scatter_add
        )
...
class Parser:
    ...
    def parse_assignment(self):
        ...
        lhs = (
            self.parse_access()
        )  # Left-hand side must be an access (e.g., Out2[i, P2[j]])
        if self.current() == "=":
            self.consume("=")
            rhs = self.parse_expr()
...
#         args=[mat1, mat2], kwargs={}, broadcast=True, type_promotion_kind=None, convert_input_to_bool=False
#     )  # Handles broadcasting the arguments
#     mul_pointwise = make_pointwise(ops.dot)(*args)
#     new_reduction = make_reduction("dot")(
#         mul_pointwise,
#         dim,
#     )
...
#     check = False
#     output_size, inner_fn, _ = index_impl_helper(x, indices, check, wrap_neg=False)
